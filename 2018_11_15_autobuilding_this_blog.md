...
environment, there was also my local theme and a clone of the [Pelican plugins repository][3]. The generated HTML was a submodule
of the source repository and was placed in the `output` directory. I used the following command to generate HTML each time I created a new post
...
With all of this, I had a `requirements.txt` document so that I could theorectically generate a post from anywhere, but I failed to keep
that updated an in sync with my development environment. On more than one occassion, I tried to write a post on my laptop (not my usual
development machine) and failed due to the mismatched dependancies, missing theme and missing plugins. 
...
directory on more than one occassion, which was annoying. 
...
I set out to change all of this with the following goals:
 ...
 - It'd be nice to have true seperation between the content of the site (the MarkDown articles) and the source code
...
[GitHub issue][5] hasn't been worked on yet. The two issues identified still persist:
 ...
 - Secure variables are transfered to a third party if the repository is transfered 
...
to the repository that contains generated HTML. I have choosen to do this because I am the only one that will be committing
to the repository and will not be transfering it to a 3rd party. This is a personal site. 
...
pull out the `content` and `output` directories and add in the theme and plugins that had only exisited locally. I also updated `requirements.txt`
to contain all of the dependancies I needed to generate the site correctly. I am pretty embarassed to admit that I hadn't updated this file in almost
three years, except to fix a plugin that had broken. 

I had Pelican pinned to an old version that I hadn't used in over a year and I was missing dependancies that the Pelican plugins required. This took
...
With the three repositories set up and content commited to them as appropriate, the next step is setting up the single Travis CI environment 
...
no reason this blog wouldn't generate against 3.6 or 3.7. I'm pretty sure all of the dependancies will also work against 2.7 
right now too. 
...
it worked. Now, I can keep the two seperate and easily determine where everything is. Template changes are source code, article
